...
if is_deepspeed_available(): #TODO(jeffra): add min version check later 
    from deepspeed.moe.layer import MoE 
    # Note that below will crash if there is an availale deepspeed that does not have ds_linear.
...
# Copied from transformers.models.llama.modeling_llama.LlamaRMSNorm with Llama->Yak
...
# Copied from transformers.models.llama.modeling_llama.LlamaRotaryEmbedding with Llama->Yak
...
# Copied from transformers.models.mistral.modeling_mistral.MistralAttention with Mistral->Yak
class YakAttention(nn.Module):
    ...
    def __init__(self, config: YakConfig, layer_idx: Optional[int] = None,  **kwargs):
        super().__init__()
        self.config = config
        ...
        self.q_proj = get_yak_linear(self.hidden_size, self.num_heads // config.tp_size * self.head_dim, bias=False,
        ...
        self.k_proj = get_yak_linear(self.hidden_size, self.num_key_value_heads // config.tp_size * self.head_dim, bias=False,
                                     ds_optimized_lora_config=deepspeed_lora_config, 
                                     ds_optimized_quantization_config=deepspeed_quantization, 
                                     ds_optimized_base_weight_sharding=True,
                                     dtype=torch.bfloat16)
        self.v_proj = get_yak_linear(self.hidden_size, self.num_key_value_heads // config.tp_size * self.head_dim, bias=False,
                                     ds_optimized_lora_config=deepspeed_lora_config, 
                                     ds_optimized_quantization_config=deepspeed_quantization, 
                                     ds_optimized_base_weight_sharding=True,
                                     dtype=torch.bfloat16)
        self.o_proj = get_yak_linear(self.hidden_size, self.num_key_value_heads // config.tp_size * self.head_dim, bias=False,
                                     ds_optimized_lora_config=deepspeed_lora_config, 
                                     ds_optimized_quantization_config=deepspeed_quantization, 
                                     ds_optimized_base_weight_sharding=True,
                                     dtype=torch.bfloat16)
...
# Copied from transformers.models.mistral.modeling_mistral.MistralFlashAttention2 with Mistral->Yak
class YakFlashAttention2(YakAttention):
    """
    Yak flash attention module. This module inherits from `YakAttention` as the weights of the module stays
...
# Copied from transformers.models.llama.modeling_llama.LlamaSdpaAttention with Llama->Yak
class YakSdpaAttention(YakAttention):
    """
    Yak attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
    `YakAttention` as the weights of the module stays untouched. The only changes are on the forward pass to adapt to
    SDPA API.
    """

    # Adapted from YakAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "YakModel is using YakSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
MIXTRAL_ATTENTION_CLASSES = {
    "eager": YakAttention,
    "flash_attention_2": YakFlashAttention2,
    "sdpa": YakSdpaAttention,
}
...
class YakMLP(nn.Module):
    def __init__(self, config: YakConfig, 
        ...
        """MLP class for Yak supporting vanilla linear layers as well as some deepspeed optimizations.

        ds_optimized_lora_config: config of type ds_linear.LoRAConfig that contains lora specific parameter if we want to add lora to this layer.
        ds_optimized_quantization_config: config of type ds_linear.QuantizationConfig.
        ds_optimized_base_weight_sharding: bool. If true, the base weight for lora (provided ds_optimized_lora_config is not None) will be sharded across all available gpus
        in a tensor parallel way.
        is_residual_mlp: bool. If true, this is MLP inside yak residual layer which has ffn_dim the same as full intermediate_size.
        """        
...
class YakMoE(nn.Module):
    def __init__(self, config: YakConfig, layer_id: int, **kwargs):
        ...
        if not self.is_moe_layer: # dense, not MoE
            ...
            if self.use_deepspeed_moe: # DeepSpeed's MoE         
...
class YakDecoderLayer(nn.Module):
    def __init__(self, config: YakConfig, layer_idx: int, **kwargs):
        super().__init__()
        self.layer_idx = layer_idx
        self.hidden_size = config.hidden_size
...
@add_start_docstrings(
    "The bare Yak Model outputting raw hidden-states without any specific head on top.",
    YAK_START_DOCSTRING,
)
# Copied from transformers.models.mistral.modeling_mistral.MistralPreTrainedModel with Mistral->Yak
class YakPreTrainedModel(PreTrainedModel):
    config_class = YakConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["YakDecoderLayer"]
...
# Copied from transformers.models.mistral.modeling_mistral.MistralModel with MISTRAL->MIXTRAL,Mistral->Yak
class YakModel(YakPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`YakDecoderLayer`]

    Args:
        config: YakConfig
    """

    def __init__(self, config: YakConfig, **kwargs):
        ...
        self.layers = nn.ModuleList(
            [YakDecoderLayer(config, layer_idx, **kwargs) for layer_idx in range(config.num_hidden_layers)]
        )
        ...
        if attention_mask is not None and self._attn_implementation == "flash_attention_2" and use_cache:
            ...
            if is_padding_right:
                raise ValueError(
                    ...
                    " this may lead to unexpected behaviour for Flash Attention version of Yak. Make sure to "
...
class YakForCausalLM(YakPreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        super().__init__(config)
        self.model = YakModel(config, **kwargs)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        ```python
        >>> from transformers import AutoTokenizer, YakForCausalLM

        >>> model = YakForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
...
@add_start_docstrings(
    """
    The Yak Model transformer with a sequence classification head on top (linear layer).

    [`YakForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->Yak, LLAMA->MIXTRAL
class YakForSequenceClassification(YakPreTrainedModel):
